...
# Fix host-provided SSL paths when running inside a Linux container.
if [ -n "$SSL_CERT_FILE" ] && [ ! -f "$SSL_CERT_FILE" ] && [ -f /etc/ssl/certs/ca-certificates.crt ] ; then
    SSL_CERT_FILE=/etc/ssl/certs/ca-certificates.crt
    export SSL_CERT_FILE
fi
if [ -n "$CURL_CA_BUNDLE" ] && [ ! -f "$CURL_CA_BUNDLE" ] && [ -f /etc/ssl/certs/ca-certificates.crt ] ; then
    CURL_CA_BUNDLE=/etc/ssl/certs/ca-certificates.crt
    export CURL_CA_BUNDLE
fi
if [ -n "$GIT_SSL_CAINFO" ] && [ ! -f "$GIT_SSL_CAINFO" ] && [ -f /etc/ssl/certs/ca-certificates.crt ] ; then
    GIT_SSL_CAINFO=/etc/ssl/certs/ca-certificates.crt
    export GIT_SSL_CAINFO
fi
if [ -n "$SSL_CERT_DIR" ] && [ ! -d "$SSL_CERT_DIR" ] && [ -d /etc/ssl/certs ] ; then
